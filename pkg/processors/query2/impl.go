...
func readView(ctx context.Context, appDef appdef.IAppDef, viewRecords istructs.IViewRecords, params QueryParams) (objects []map[string]interface{}, err error) {
	objects, err = read(ctx, appDef, viewRecords, params)
	if err != nil {
		return
	}
...
func read(ctx context.Context, appDef appdef.IAppDef, viewRecords istructs.IViewRecords, params QueryParams) (objects []map[string]interface{}, err error) {
	kb := viewRecords.KeyBuilder(params.Entity)
	kb.PutFromJSON(params.Constraints.Where)
	objects = make([]map[string]interface{}, 0)
	err = viewRecords.Read(ctx, params.WSID, kb, func(key istructs.IKey, value istructs.IValue) (err error) {
